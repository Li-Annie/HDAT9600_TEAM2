...
We selected the predictor variables (age, length of stay, ICU type, SOFA score, fractional inspired oxygen (FiO2), Glasgow Coma Score (GCS), heart rate and respiration rate) to predict the risk of in-hospital death.  
...
Furthermore, "Heart rate at admission is a predictor of in-hospital mortality in patients with acute coronary syndromes" from PubMed propose that heart rate can be associated with in-hospital mortality in certain sub-population of patients. 

Lastly, "Mean nocturnal respiratory rate predicts cardiovascular and all-cause mortality in community-dwelling older men and women" from ERS reports that the association between respiratory rate and in hospital and short term mortality. 


```{r Task1-EDAii}
# Subsetting the ICU dataset into variables of interest
icu_sub <- icu_patients_df1 %>% dplyr::select(in_hospital_death, ICUType, Age, Length_of_stay, SOFA, FiO2_max, RespRate_min, HR_min, GCS_max)

# Examining the structure of the subset of dataset
str(icu_sub)

```
The subsetted ICU dataset contains 2061 observations of 9 variables. The outcome variable is death in hospital and the predictor variables are type of ICU, age, length of stay, SOFA scores, FiO2, respiration rate, heart rate and GCS.
...
```{r, Task1-HR}
HR_glm <- glm(in_hospital_death ~ HR_min, family = binomial, data = icu_sub)
summary(HR_glm)
```

```{r, Task1-GCS}
GCS_glm <- glm(in_hospital_death ~  GCS_max, family = binomial, data = icu_sub)
summary(GCS_glm)
```
...
ggplot(aes(x=predicted_probability_bin, 
  ...
  geom_point() + geom_linerange(colour="blue", alpha=0.3) + 
...
ggplot(aes(x=predicted_probability_bin, 
           ...
           ymin=in_hospital_death_bin/n_bin - 1.96*se_predicted_probability_bin,
           ymax=in_hospital_death_bin/n_bin + 1.96*se_predicted_probability_bin)) +
  geom_point() + geom_linerange(colour="blue", alpha=0.5) +
  labs(x="Predicted probability (binned)",
