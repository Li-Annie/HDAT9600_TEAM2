...
```{r, Task1-Pseudo-R2-reduced}
options(scipen=999)
# Checking the fit of the reduced model using Pseudo-R2
PseudoR2(aic_suggest_mod, which = "all")
```
_The Pseudo-R2 do not differ by that much from the full model and the reduced model. When the fit of the model is very similar, the reduced/simpler model is always preferred._  
...
# Checking the goodness of fit of full model using Brier Score test
pred_prob_full <- predict(full_mod, type="response")
brier_score_full <- mean((pred_prob_full - icu_sub$in_hospital_death)^2)
brier_score_full
...
brier_score_reduced
```
_The Brier score also shows the full model is a better fit like the Pseudo-R2. Again, as the difference is very small, the reduced/simpler model is preferred._  

```{r, Task1-Diagnostic plots}
# Diagnostic plots of full model
icu_sub %>% mutate(predicted_probability = predict(full_mod, type = "response"), linear_predictor = predict(full_mod)) %>%
 group_by(cut(linear_predictor, breaks=unique(quantile(linear_predictor, (1:50)/51)))) %>%
summarise(in_hospital_death_bin=sum(in_hospital_death), predicted_probability_bin=mean(predicted_probability), n_bin=n()) %>%
mutate(se_predicted_probability_bin=sqrt(predicted_probability_bin*(1 - predicted_probability_bin)/n_bin)) %>%
ggplot(aes(x=predicted_probability_bin, 
           y=in_hospital_death_bin/n_bin, 
           ymin=in_hospital_death_bin/n_bin - 1.96*se_predicted_probability_bin,
           ymax=in_hospital_death_bin/n_bin + 1.96*se_predicted_probability_bin)) +
  geom_point() + geom_linerange(colour="blue", alpha=0.3) + labs(x="Predicted probability (binned)",
       y="Observed proportion (in each bin)", title = "Diagnostic Plot of Full model") + geom_abline(intercept=0, slope=1)

# Diagnostic plots of reduced model
icu_sub %>% mutate(predicted_probability = predict(aic_suggest_mod, type = "response"), linear_predictor = predict(aic_suggest_mod)) %>%
 group_by(cut(linear_predictor, breaks=unique(quantile(linear_predictor, (1:50)/51)))) %>%
summarise(in_hospital_death_bin=sum(in_hospital_death), predicted_probability_bin=mean(predicted_probability), n_bin=n()) %>%
mutate(se_predicted_probability_bin=sqrt(predicted_probability_bin*(1 - predicted_probability_bin)/n_bin)) %>%
ggplot(aes(x=predicted_probability_bin, 
           ...
           ymin=in_hospital_death_bin/n_bin - 1.96*se_predicted_probability_bin,
           ymax=in_hospital_death_bin/n_bin + 1.96*se_predicted_probability_bin)) +
  geom_point() + geom_linerange(colour="blue", alpha=0.5) + labs(x="Predicted probability (binned)",
       y="Observed proportion (in each bin)", title = "Diagnostic Plot of Reduced model") + geom_abline(intercept=0, slope=1) 
```
