...
icu_sub <- icu_patients_df1 %>% select(in_hospital_death, ICUType, Age, Length_of_stay, SOFA, FiO2_max, RespRate_min, HR_min, GCS_max)
...
#
icu_sub$in_hospital_death <- ifelse(icu_sub$in_hospital_death == 0, "Survivor", "Died in Hospital")
...
plots <- for (column in names) {
  print(ggplot(data = icu_sub, aes(x = icu_sub[,column], fill = in_hospital_death))+ geom_histogram() + theme_minimal() + scale_x_continuous(expand = c(0,0)) + scale_y_continuous(expand = c(0,0)) + ggtitle(paste("Histogram of", column)) + labs(x = column) + facet_grid(in_hospital_death~., scales = "fixed"))
  }
