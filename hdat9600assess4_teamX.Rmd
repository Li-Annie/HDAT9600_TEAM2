...
-   important vital signs: `HR_min`, `RespRate_min`, `SaO2_max`,
    `SysABP_max`, `Temp_max`, `GCS_max`, `Urine_max`
...
```{r coxmod_mv1}
coxmod_mv_all <- coxph(surv_object ~ Age + HR_min + RespRate_min + SaO2_max + SysABP_max + Temp_max + GCS_max + Urine_max + Lactate_max + HCT_min + Creatinine_max + Na_max, data=icu_sub2)
coxmod_mv_uni <- coxph(surv_object ~ Age + RespRate_min + SysABP_max + Temp_max + GCS_max + Urine_max + Lactate_max + Creatinine_max, data=icu_sub2)
```

```{r coxmod_mv_all}
summary(coxmod_mv_all)
```

```{r coxmod_mv_uni}
summary(coxmod_mv_uni)
```
