...
plots <- for (i in 3:ncol(icu_sub)) {
  names <- colnames(icu_sub[-c(1:2)])
  for (column in names) {
  print(ggplot(data = icu_sub, aes(x = icu_sub[,i], fill = in_hospital_death))+ geom_histogram() + theme_minimal() + scale_x_continuous(expand = c(0,0)) + scale_y_continuous(expand = c(0,0)) + ggtitle(paste("Histogram of", column)) + labs(x = column) + facet_grid(in_hospital_death~., scales = "fixed"))
  }
}
